...
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v13.json';
...
d.name = 'Fluxo Victor Pizza v13 - anti-duplicata via filtro de telefones';
...
// 5a. buscar_fones_existentes — GET Supabase, retorna telefones ja salvos
var nodeFones = {
...
console.log('✓ buscar_fones_existentes adicionado (GET Supabase telefones)');

// 5b. preparar_busca — monta URL CNPJa (sem offset, sem city filter)
var nodePreparar = {
  id: crypto.randomUUID(),
  name: 'preparar_busca',
...
console.log('✓ preparar_busca adicionado');
...
// 5d. processar_leads — filtra, normaliza e remove duplicatas
var nodeProcessar = {
  id: crypto.randomUUID(),
...
console.log('✓ processar_leads adicionado (com filtro de duplicatas por telefone)');

// ── 6. Cabeamento ─────────────────────────────────────────────────────────────
d.connections['extrair_params'] = {
  main: [[{ node: 'buscar_fones_existentes', type: 'main', index: 0 }]]
};
d.connections['buscar_fones_existentes'] = {
...
console.log('✓ Cadeia: extrair_params → buscar_fones_existentes → preparar_busca → buscar_cnpja → processar_leads → finalizar_busca');
...
console.log('\n=== VERIFICACAO FINAL ===');
var checks = [
  ['Nome v13',                    json.includes('Victor Pizza v13')],
  ...
  ['SEM offset=',                 !json.includes('"offset=')],
  ['SEM address.city.in',         !json.includes('address.city.in')],
  ['Supabase telefones',          json.includes('supabase_phones_url')],
  ...
  ['Filtro de duplicatas',        json.includes('telefonesExistentes')],
];
...
function buildExtrairParams() {
  ...
  L.push("");
  L.push("// URL do Supabase para buscar todos os telefones ja salvos deste tipo_loja");
  L.push("// Usamos select=telefone para trazer apenas o campo necessario");
...
function buildPreparar() {
  var L = [];
  L.push("// Monta URL do CNPJa — so filtros confirmados como validos");
  L.push("// (offset e address.city.in removidos: causavam 'request validation failed')");
...
function buildProcessar() {
  ...
  L.push("}");
  L.push("");
  L.push("// Carrega telefones ja existentes no banco (vindos do buscar_fones_existentes)");
  L.push("// O Supabase retorna [{telefone:'...'}, ...] — n8n divide em itens separados");
  ...
  L.push("    }");
  L.push("  }");
  L.push("} catch(e) { /* sem leads existentes */ }");
  L.push("");
  ...
  L.push("");
  L.push("  // Filtro de cidade no codigo (CNPJa nao aceita address.city.in na busca)");
  ...
  L.push("");
  L.push("  // Anti-duplicata: pula telefones ja existentes no banco");
  L.push("  if (telefonesExistentes[cel] || telefonesExistentes['55' + cel]) continue;");
  L.push("");
  L.push("  // Busca email: CORPORATE → PERSONAL → qualquer disponivel");
