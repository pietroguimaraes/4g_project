...
// v25: corrige o body do email — usa expressao pura (= no inicio) igual ao subject
// O subject funcionou porque e uma expressao pura. O body nao funcionava porque
// {{ }} inline nao avalia referencias a nodes pelo nome ($('node').item.json)
const INPUT  = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v23.json';
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v25.json';

var d = JSON.parse(fs.readFileSync(INPUT, 'utf8'));
d.name = 'Fluxo Victor Pizza v25 - fix body email (expressao pura)';
...
// Expressao que resolve o nome da empresa (igual ao subject que ja funcionou)
var empresa = "$('garantir_campos').item.json.Empresa || $('garantir_campos').item.json.empresa";

// Body como expressao pura: = 'parte html' + (expressao) + 'resto html'
// Cada parte e uma string concatenada — o = no inicio faz o n8n avaliar tudo
var bodyExpr = "="
  + "'<!DOCTYPE html>'"
  + " + '<html>'"
...
gmailNode.parameters.message = bodyExpr;
console.log('OK Body atualizado como expressao pura (= no inicio)');
...
console.log('\n=== INSTRUCOES ===');
console.log('1. Importa Fluxo_victor_pizza_v25.json no n8n');
console.log('2. Confirma credencial Gmail em enviar_email_gmail');
console.log('3. Ativa e testa — o nome da empresa deve aparecer corretamente no email');
