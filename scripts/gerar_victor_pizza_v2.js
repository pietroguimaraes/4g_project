...
// Output: fluxo Victor Pizza v2 — CNPJ (BrasilAPI) + validação SMTP (mailcheck.ai)
const INPUT  = 'C:/Users/guima/Downloads/Fluxo_oriental_limpeza_v11.json';
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v2.json';

const d = JSON.parse(fs.readFileSync(INPUT, 'utf8'));

// ─── 1. Nome do fluxo ─────────────────────────────────────────────────────────
d.name = 'Fluxo Victor Pizza v2 — supermercados SP/PR/RS/SC + CNPJ + validação email';
console.log('✓ Nome atualizado');

// ─── 2. enriquecer_leads: CNPJ + validação SMTP ───────────────────────────────
...
enriquece.parameters.jsCode = `// v2-victor-pizza — enriquecer_leads com CNPJ (BrasilAPI) + validação SMTP (mailcheck.ai)
...
// NOVO: buscar email oficial via CNPJ na BrasilAPI (grátis, sem chave)
...
// NOVO: validar email via mailcheck.ai (grátis, sem API key)
async function validarEmail(email) {
  ...
  try {
    ...
    // Aceita se: tem registro MX, não é descartável, não está bloqueado
...
// Busca contatos com waterfall de email: IA → CNPJ BrasilAPI → validação SMTP
...
3. CNPJ da empresa (busque no Google):
  ...
  // Waterfall de email: se IA não encontrou mas tem CNPJ, tenta BrasilAPI
  if (!email && cnpj) {
  ...
  // Validação SMTP: só entrega email se passar na verificação
  if (email) {
    const valido = await validarEmail(email);
    if (!valido) {
      email = null; // descarta email inválido — Victor nunca vê email morto
    }
  }
...
console.log('✓ enriquecer_leads v2: CNPJ BrasilAPI + validação SMTP adicionados');

// ─── 3. edit_fields: garante campo email no mapeamento ────────────────────────
...
console.log('\n=== VERIFICAÇÃO ===');
console.log('Nome atualizado:       ', json.includes('Victor Pizza v2') ? '✓' : '✗');
