...
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v14.json';
...
d.name = 'Fluxo Victor Pizza v14 - IBGE municipio + fixo aceito';
...
console.log('✓ Referencias enriquecer_leads → extrair_params');

// ── 3. FIX: $('enriquecer_leads') → $('processar_leads') ─────────────────────
...
// ── 5. Adiciona os 6 novos nos ────────────────────────────────────────────────
var px = posBase[0];
var py = posBase[1];

// 5a. buscar_municipios_ibge — lista todos municípios do estado
var nodeMunicipios = {
  id: crypto.randomUUID(),
  name: 'buscar_municipios_ibge',
  type: 'n8n-nodes-base.httpRequest',
  typeVersion: 4.1,
  position: [px + 220, py],
  parameters: {
    method: 'GET',
    url: '={{ "https://servicodados.ibge.gov.br/api/v1/localidades/estados/" + $json.estado + "/municipios?orderBy=nome" }}',
    authentication: 'none',
    sendQuery: false,
    sendHeaders: false,
    sendBody: false,
...
d.nodes.push(nodeMunicipios);
console.log('✓ buscar_municipios_ibge adicionado');

// 5b. encontrar_id — extrai código IBGE da cidade pedida
var nodeEncontrarId = {
  id: crypto.randomUUID(),
  name: 'encontrar_id',
  ...
  typeVersion: 2,
  position: [px + 440, py],
  parameters: { mode: 'runOnceForAllItems', jsCode: buildEncontrarId() }
};
d.nodes.push(nodeEncontrarId);
console.log('✓ encontrar_id adicionado');

// 5c. buscar_fones_existentes — telefones já no banco para anti-duplicata
var nodeFones = {
  id: crypto.randomUUID(),
...
console.log('✓ buscar_fones_existentes adicionado');

// 5d. preparar_busca — monta URL CNPJa com municipio IBGE
var nodePreparar = {
  id: crypto.randomUUID(),
  name: 'preparar_busca',
  type: 'n8n-nodes-base.code',
...
console.log('✓ preparar_busca adicionado (usa municipio IBGE)');

// 5e. buscar_cnpja — HTTP GET CNPJa
var nodeBuscarCnpja = {
  id: crypto.randomUUID(),
  name: 'buscar_cnpja',
  type: 'n8n-nodes-base.httpRequest',
...
// 5f. processar_leads — normaliza, filtra duplicatas, aceita fixo e celular
var nodeProcessar = {
  id: crypto.randomUUID(),
...
console.log('✓ processar_leads adicionado (aceita fixo + celular)');

// ── 6. Cabeamento ─────────────────────────────────────────────────────────────
d.connections['extrair_params']         = { main: [[{ node: 'buscar_municipios_ibge', type: 'main', index: 0 }]] };
d.connections['buscar_municipios_ibge'] = { main: [[{ node: 'encontrar_id',           type: 'main', index: 0 }]] };
d.connections['encontrar_id']           = { main: [[{ node: 'buscar_fones_existentes',type: 'main', index: 0 }]] };
...
console.log('✓ Cadeia: extrair_params → buscar_municipios_ibge → encontrar_id → buscar_fones_existentes → preparar_busca → buscar_cnpja → processar_leads → finalizar_busca');
...
if (editFields) {
  ...
  } else { console.log('✓ edit_fields: email ja presente'); }
...
console.log('\n=== VERIFICACAO FINAL ===');
var checks = [
  ['Nome v14',                    json.includes('Victor Pizza v14')],
  ['extrair_params',              json.includes('"extrair_params"')],
  ['buscar_municipios_ibge',      json.includes('"buscar_municipios_ibge"')],
  ['encontrar_id',                json.includes('"encontrar_id"')],
  ...
  ['IBGE municipios URL',         json.includes('servicodados.ibge.gov.br')],
  ['municipio_id no preparar',    json.includes('municipio_id')],
  ['address.municipality.in',     json.includes('address.municipality.in')],
  ['emails.ex=true',              json.includes('emails.ex=true')],
  ['Aceita fixo (LANDLINE ok)',   json.includes('LANDLINE_OK')],
  ['Anti-duplicata',              json.includes('telefonesExistentes')],
...
function buildExtrairParams() {
  ...
  L.push("");
  L.push("var supabase_phones_url = '" + SUPABASE_URL + "/rest/v1/leads'");
...
function buildEncontrarId() {
  var L = [];
  L.push("// Recebe todos os municípios do estado (array do IBGE)");
  L.push("// Encontra o código IBGE da cidade pedida pelo usuário");
  L.push("function normCidade(s) {");
  L.push("  if (!s) return '';");
  ...
  L.push("var cidadeAlvo = normCidade(params.cidade);");
  L.push("var municipios = $input.all();");
  L.push("var municipioId = null;");
  L.push("");
  L.push("for (var i = 0; i < municipios.length; i++) {");
  L.push("  var item = municipios[i].json;");
  L.push("  if (normCidade(item.nome) === cidadeAlvo) {");
  L.push("    municipioId = item.id;");
  L.push("    break;");
  L.push("  }");
  L.push("}");
  L.push("");
  L.push("console.log('[encontrar_id] cidade=' + params.cidade + ' id=' + municipioId);");
  L.push("");
  L.push("return [{ json: {");
  ...
  L.push("  cnaes_str:           params.cnaes_str,");
  L.push("  limit:               params.limit,");
  L.push("  supabase_phones_url: params.supabase_phones_url,");
  L.push("  municipio_id:        municipioId");
  L.push("} }];");
  return L.join('\n');
}
...
function buildPreparar() {
  var L = [];
  L.push("// Monta URL do CNPJa com filtro de municipio IBGE (address.municipality.in)");
  L.push("// O municipio_id veio do encontrar_id via IBGE API");
  L.push("var params = $('encontrar_id').first().json;");
  L.push("");
  ...
  L.push("];");
  L.push("");
  L.push("// Adiciona filtro de municipio IBGE se encontrado");
...
function buildProcessar() {
  ...
  L.push("}");
  L.push("");
  L.push("// Aceita celular (11 dígitos, 3o digito=9) E fixo (10 dígitos)");
  L.push("// LANDLINE_OK — marcador para verificacao");
  ...
  L.push("  // Celular: 11 dígitos, 3o = 9");
  L.push("  if (digits.length === 11 && digits.charAt(2) === '9') return digits;");
  L.push("  // Celular antigo sem o 9: 10 dígitos começando com 6-9 → adiciona 9");
  L.push("  if (digits.length === 10 && '6789'.indexOf(digits.charAt(2)) >= 0) {");
  L.push("    return digits.substring(0, 2) + '9' + digits.substring(2);");
  L.push("  }");
  L.push("  // Fixo: 10 dígitos começando com 2-5");
  L.push("  if (digits.length === 10 && '2345'.indexOf(digits.charAt(2)) >= 0) {");
  L.push("    return digits; // aceita fixo como está (10 dígitos)");
  L.push("  }");
  L.push("  return null;");
  L.push("}");
  L.push("");
  L.push("// Anti-duplicata: carrega telefones já no banco");
