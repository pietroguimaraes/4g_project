...
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v11.json';

const CNPJA_KEY = '1239882e-5005-475f-9be1-41b4123846dd-b7c95f5e-5c9e-4b78-9ef3-4ffdb872a793';

const d = JSON.parse(fs.readFileSync(INPUT, 'utf8'));
d.name = 'Fluxo Victor Pizza v11 - CNPJa com email e filtro por cidade';
...
console.log('✓ preparar_busca configurado (emails.ex=true + filtro por cidade)');
...
console.log('✓ Referencias enriquecer_leads → preparar_busca nas conexoes atualizadas');

// ── 3. FIX: substitui $('enriquecer_leads') em TODOS os parametros dos nos ───
var fixCount = 0;
...
// ── 4. Bypassa definir_termos/Apify ───────────────────────────────────────────
...
for (var ni = 0; ni < nomesOrigem.length; ni++) {
  ...
  for (var oi = 0; oi < conn.main.length; oi++) {
    ...
    if (temApify) {
      ...
      console.log('✓ Reconectado: ' + nomeCon + '[' + oi + '] → preparar_busca');
      reconectado = true;
      break;
    }
  }
...
console.log('✓ processar_leads adicionado (com filtro de cidade)');

// ── 6. Cabeamento: preparar_busca → buscar_cnpja → processar_leads → finalizar
...
console.log('\n=== VERIFICACAO FINAL ===');
var checks = [
  ['Nome v11',                  json.includes('Victor Pizza v11')],
...
function buildPreparar() {
  ...
  L.push("// Busca mais do que o necessario para compensar filtros e nulos");
  ...
  L.push("];");
  L.push("");
  L.push("// Filtra por cidade se informada (CNPJa armazena cidades em maiusculas)");
  L.push("if (cidade) {");
  L.push("  params.push('address.city.in=' + encodeURIComponent(cidade.toUpperCase()));");
  L.push("}");
  L.push("");
  L.push("return [{ json: {");
...
function buildProcessar() {
  ...
  L.push("}");
  L.push("");
  L.push("// Remove acentos para comparar cidade (CNPJa usa maiusculas sem acento)");
  ...
  L.push("");
  L.push("  // Filtra por cidade se informada (comparacao normalizada)");
  ...
  L.push("");
  L.push("  // Busca email: prioridade CORPORATE depois PERSONAL");
