...
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v8.json';
...
d.name = 'Fluxo Victor Pizza v8 - CNPJa via HTTP Request';
...
// processar_leads: Code (filtra e normaliza todos os registros)
var nodeProcessar = {
  id: crypto.randomUUID(),
...
// FIX v8: verificar_whatsapp usa bodyParameters (JSON) em vez de raw body
// Isso resolve o 405 Method Not Allowed da UazAPI
var nodeVerificarWA = {
  id: crypto.randomUUID(),
  ...
  parameters: {
    ...
    bodyParameters: {
      parameters: [
        { name: 'phone', value: '={{ $json._wa_phone }}' }
      ]
    },
    options: {}
  }
...
console.log('✓ verificar_whatsapp adicionado (FIX: contentType json + bodyParameters)');

// FIX v8: juntar_e_filtrar usa runOnceForAllItems
// Resolve "Code doesn't return a single object" ao descartar itens sem WA
var nodeJuntar = {
  id: crypto.randomUUID(),
...
console.log('✓ juntar_e_filtrar adicionado (FIX: runOnceForAllItems + pairing por indice)');
...
console.log('\n=== VERIFICACAO FINAL ===');
var checks = [
  ['Nome v8',              json.includes('Victor Pizza v8')],
  ...
  ['bodyParameters',       json.includes('bodyParameters')],
  ['runOnceForAllItems',   json.includes('runOnceForAllItems')],
  ['Sem $helpers',         !json.includes('$helpers')],
  ['Sem fetch(',           !json.includes('fetch(')],
  ['Sem raw body',         !json.includes('rawContentType')],
...
function buildJuntar() {
  var L = [];
  // FIX v8: runOnceForAllItems — acessa todos os resultados WA e todos os leads por indice
  // Resolve: "Code doesn't return a single object [item 0]" do modo runOnceForEachItem
  L.push("// juntar_e_filtrar — combina resultados WA com leads por indice");
  L.push("// FIX v8: runOnceForAllItems evita erro de array vazio no modo por-item");
  L.push("var waResults = $input.all();");
  L.push("var leads     = $('processar_leads').all();");
  L.push("");
  L.push("var resultado = [];");
  L.push("");
  L.push("for (var i = 0; i < leads.length; i++) {");
  L.push("  var lead   = leads[i].json;");
  L.push("  var waResp = waResults[i] ? waResults[i].json : {};");
  L.push("");
  L.push("  // Passa sem_resultado direto");
  L.push("  if (lead._sem_resultado) {");
  L.push("    resultado.push({ json: lead });");
  L.push("    continue;");
  L.push("  }");
  L.push("");
  L.push("  // Descarta se nao esta no WhatsApp (ou se houve erro na checagem)");
  L.push("  if (waResp.exists !== true) continue;");
  L.push("");
  L.push("  resultado.push({ json: Object.assign({}, lead, { _wa_verificado: true }) });");
  L.push("}");
  L.push("");
  L.push("if (resultado.length === 0) {");
  L.push("  return [{ json: { _sem_resultado: true, _meta_bruta: 0, _meta_filtrada: 0 } }];");
  L.push("}");
  L.push("");
  L.push("return resultado;");
  return L.join('\n');
}
