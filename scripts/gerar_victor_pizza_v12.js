...
const OUTPUT = 'C:/Users/guima/Downloads/Fluxo_victor_pizza_v12.json';
...
d.name = 'Fluxo Victor Pizza v12 - CNPJa com offset anti-duplicata';
...
console.log('✓ extrair_params configurado');

// ── 2. Atualiza referencias de conexoes enriquecer_leads → extrair_params ─────
...
console.log('✓ Referencias enriquecer_leads → extrair_params nas conexoes');

// ── 3. FIX: substitui $('enriquecer_leads') em parametros dos nos ─────────────
var fixCount = 0;
d.nodes.forEach(function(node) {
  ...
  if (paramStr.indexOf('enriquecer_leads') >= 0) {
    ...
    console.log('✓ Fix enriquecer_leads → processar_leads em: ' + node.name);
    fixCount++;
  }
});
if (fixCount === 0) console.log('  (nenhum no referenciava enriquecer_leads nos parametros)');

// ── 4. Reconecta reserva_suficiente → extrair_params (bypassa Apify) ──────────
...
for (var ni = 0; ni < nomesOrigem.length; ni++) {
  ...
  for (var oi = 0; oi < conn.main.length; oi++) {
    ...
    if (temApify) {
      ...
      console.log('✓ Reconectado: ' + nomeCon + '[' + oi + '] → extrair_params');
      reconectado = true;
      break;
    }
  }
...
// 5a. contar_existentes — GET Supabase com Prefer: count=exact
var nodeContar = {
  id: crypto.randomUUID(),
  name: 'contar_existentes',
  type: 'n8n-nodes-base.httpRequest',
  typeVersion: 4.1,
  position: [px + 220, py],
  parameters: {
    method: 'GET',
    url: '={{ $json.supabase_count_url }}',
    authentication: 'none',
    sendQuery: false,
    sendHeaders: true,
    headerParameters: {
...
d.nodes.push(nodeContar);
console.log('✓ contar_existentes adicionado (GET Supabase com count=exact)');

// 5b. preparar_busca — combina params + offset → URL CNPJa
var nodePreparar = {
  id: crypto.randomUUID(),
  name: 'preparar_busca',
...
console.log('✓ preparar_busca adicionado (monta URL com offset)');

// 5c. buscar_cnpja — HTTP GET na API do CNPJa
var nodeBuscarCnpja = {
  id: crypto.randomUUID(),
  name: 'buscar_cnpja',
  type: 'n8n-nodes-base.httpRequest',
...
// 5d. processar_leads — filtra e normaliza resultados do CNPJa
var nodeProcessar = {
  id: crypto.randomUUID(),
...
// ── 6. Cabeamento completo ────────────────────────────────────────────────────
d.connections['extrair_params'] = {
  main: [[{ node: 'contar_existentes', type: 'main', index: 0 }]]
};
d.connections['contar_existentes'] = {
  main: [[{ node: 'preparar_busca', type: 'main', index: 0 }]]
};
...
console.log('✓ Cadeia: extrair_params → contar_existentes → preparar_busca → buscar_cnpja → processar_leads → finalizar_busca');

// ── 7. Corrige finalizar_busca (remove refs a definir_termos) ─────────────────
...
// ── 8. edit_fields: garante campos email e cnpj ───────────────────────────────
...
if (editFields) {
  ...
  if (!temCnpj) {
    ...
    console.log('✓ edit_fields: campo cnpj adicionado');
  }
}
...
console.log('\n=== VERIFICACAO FINAL ===');
var checks = [
  ['Nome v12',                    json.includes('Victor Pizza v12')],
  ['extrair_params',              json.includes('"extrair_params"')],
  ['contar_existentes',           json.includes('"contar_existentes"')],
  ...
  ['filtro cidade',               json.includes('address.city.in')],
  ['offset no URL',               json.includes('offset=')],
  ['Supabase count',              json.includes('count=exact')],
  ['fullResponse true',           json.includes('fullResponse')],
  ...
  ['processar_leads em patch',    json.includes("$('processar_leads')")],
...
function buildExtrairParams() {
  var L = [];
  L.push("// Le parametros do webhook e monta URL de contagem do Supabase");
  ...
  L.push("");
  L.push("// URL do Supabase para contar leads ja salvos desta cidade/tipo");
  L.push("// Usamos limit=0 + Prefer:count=exact para obter so o total no header Content-Range");
  L.push("var supabase_count_url = '" + SUPABASE_URL + "/rest/v1/leads'");
  L.push("  + '?select=id'");
  L.push("  + '&tipo_loja=eq.' + encodeURIComponent(tipo_loja)");
  L.push("  + (cidade ? '&cidade=ilike.' + encodeURIComponent(cidade) : '')");
  L.push("  + '&limit=0';");
  L.push("");
  L.push("return [{ json: {");
  ...
  L.push("  limit:              limit,");
  L.push("  supabase_count_url: supabase_count_url");
  L.push("} }];");
  return L.join('\n');
}
...
function buildPreparar() {
  var L = [];
  L.push("// Le params de extrair_params e contagem de contar_existentes");
  L.push("var params = $('extrair_params').first().json;");
  L.push("var estado            = params.estado;");
  L.push("var cidade            = params.cidade;");
  L.push("var tipo_loja         = params.tipo_loja;");
  L.push("var quantidade_pedida = params.quantidade_pedida;");
  L.push("var search_id         = params.search_id;");
  L.push("var cnaes_str         = params.cnaes_str;");
  L.push("var limit             = params.limit;");
  L.push("");
  L.push("// Le o total de leads ja existentes a partir do header Content-Range do Supabase");
  L.push("// Formato do header: '*/47' ou '0-0/47' → queremos o numero apos '/'");
  L.push("var contarResp    = $input.first().json;");
  L.push("var contentRange  = (contarResp.headers && contarResp.headers['content-range']) || '';");
  L.push("var matchRange    = contentRange.match(/\\/(\\d+)/);");
  L.push("var offset        = matchRange ? parseInt(matchRange[1]) : 0;");
  L.push("console.log('[preparar_busca] offset=' + offset + ' (leads ja existentes)');");
  L.push("");
  ...
  L.push("  'limit='  + limit,");
  L.push("  'offset=' + offset");
  L.push("];");
  L.push("");
  L.push("// Adiciona filtro de cidade se informada");
  L.push("if (cidade) {");
  L.push("  queryParams.push('address.city.in=' + encodeURIComponent(cidade.toUpperCase()));");
  L.push("}");
  L.push("");
  L.push("return [{ json: {");
...
function buildProcessar() {
  ...
  L.push("}");
  L.push("");
  L.push("function normalizarCidade(s) {");
  L.push("  if (!s) return '';");
  L.push("  return s.toUpperCase()");
  L.push("    .replace(/[ÁÀÃÂÄ]/g, 'A').replace(/[ÉÈÊË]/g, 'E')");
  L.push("    .replace(/[ÍÌÎÏ]/g, 'I').replace(/[ÓÒÕÔÖ]/g, 'O')");
  L.push("    .replace(/[ÚÙÛÜ]/g, 'U').replace(/[Ç]/g, 'C');");
  L.push("}");
  L.push("");
  L.push("var preparar          = $('preparar_busca').first().json;");
  L.push("var quantidade_pedida = preparar.quantidade_pedida;");
  L.push("var cidadePedida      = normalizarCidade(preparar.cidade);");
  ...
  L.push("");
  L.push("  // Filtro de cidade pos-API (garante correspondencia mesmo se CNPJa ignorar o filtro)");
  L.push("  if (cidadePedida) {");
  L.push("    var cidadeReg = normalizarCidade(reg.address && reg.address.city);");
  L.push("    if (cidadeReg && cidadeReg !== cidadePedida) continue;");
  L.push("  }");
  L.push("");
  ...
  L.push("");
  L.push("  // Busca email: CORPORATE → PERSONAL → qualquer");
